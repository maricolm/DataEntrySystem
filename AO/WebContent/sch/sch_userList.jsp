      ...
      <div class="am-u-sm-12">
        <form class="am-form">
          <table class="am-table am-table-striped am-table-hover table-main">
            <thead>
              <tr>
                <th class="table-title">账号</th><th class="table-title">姓名</th><th class="table-title">学校</th><th class="table-type">部门</th><th class="table-author">联系电话</th><th class="table-date">邮箱</th><th class="table-set">操作</th>
              </tr>
          </thead>
          <tbody>
          	<c:forEach items="${pb.beanList }" var="col">
          		<tr>
          		  <td>${col.cAccount }</td>
	              <td>${col.cUname }</td>
	              <td>${col.cName }</td>
	              <td>${col.cDepartment }</td>
	              <td>${col.cPhone }</td>
	              <td>${col.cEmail }</td>
	              <td>
	                <div class="am-btn-toolbar">
	                  <div class="am-btn-group am-btn-group-xs">
	                    <a id="${col.cId }"  class="reset am-btn am-btn-default am-btn-xs am-text-secondary"><span class="am-icon-pencil-square-o"></span>重置密码</a>
	                    <a href="${pageContext.request.contextPath }/sch/deleteUserByid.do?cId=${col.cId }" class="am-btn am-btn-default am-btn-xs am-text-danger" onclick="return confirm('确认删除？')"><span class="am-icon-trash-o"></span> 删除</a>
	                  </div>
	                </div>
	              </td>
	             </tr>
		    ...
		    <ul class="am-pagination">
		      <li>
		      	<c:choose>
			    	...
			    	<c:otherwise>
			    		<a href="${pageContext.request.contextPath }/sch/findUser.do?currentPage=${pb.currentPage-1}" >
					        «
					    </a>
			    	</c:otherwise>
			    </c:choose>
		     ...
		     <c:forEach begin="${pb.startPage}" end="${pb.endPage}" var="num" step="1">
		          <li ${pb.currentPage==num?"class=' am-active'":"" }><a href="${pageContext.request.contextPath }/sch/findUser.do?currentPage=${num}">${num}</a></li>
		      </c:forEach>
		     	 <li>
		      <c:choose>
...
<script src="<c:url value='assets/js/resetPswd.js'/>"></script>
</body>
</html>
