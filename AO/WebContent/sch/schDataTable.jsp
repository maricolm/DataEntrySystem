		    ...
		    <ul class="am-pagination">
		      <li>
		      	<c:choose>
			    	...
			    	<c:otherwise>
			    		<a href="${pageContext.request.contextPath }/sch/seeDataOfCol.do?pId=${pId }&cId=${cId }&currentPage=${pb.currentPage-1}" >
					        «
					    </a>
			    	</c:otherwise>
			    </c:choose>
		     ...
		     <c:forEach begin="${pb.startPage}" end="${pb.endPage}" var="num" step="1">
		          <li ${pb.currentPage==num?"class=' am-active'":"" }><a href="${pageContext.request.contextPath }/sch/seeDataOfCol.do?pId=${pId }&cId=${cId }&currentPage=${num}">${num}</a></li>
		      </c:forEach>
		     	 <li>
		      <c:choose>
