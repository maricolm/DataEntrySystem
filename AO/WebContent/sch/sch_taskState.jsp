      ...
      <div class="am-u-sm-12">
        <form class="am-form">
          <table class="am-table am-table-striped am-table-hover table-main">
            <thead>
              <tr>
                <th class="table-title">项目名</th><th class="table-title">学院名</th><th class="table-title">用户名</th><th  class="table-title">任务状态</th><th class="table-title">选择</th>
              </tr>
          </thead>
          <tbody>
          <c:forEach items="${pb.beanList }" var="tPro">
           <tr>
              <td>${tPro.pName }</td>
              <td>${tPro.tDepartment }</td>
              <td>${tPro.cName }</td>
              <td>
              	<c:choose>
              		<c:when test="${tPro.tPState==1 }">
              			<font color="red">待分发</font>
              		</c:when>
              		<c:when test="${tPro.tPState==2 && tPro.tState==1}">
              			<font color="red">待接收</font>
              		</c:when>
              		<c:when test="${tPro.tState==2 }">
              			<font color="red">已接收</font>
              		</c:when>
              		<c:when test="${tPro.tState==3 }">
              			<font color="red">待确认</font>
              		</c:when>
              		<c:when test="${tPro.tState==4}">
              			<font color="red">已确认</font>
              		</c:when>
              		<c:when test="${tPro.tState==5 }">
              ...
              <td>
                <div class="am-btn-toolbar">
                  ...
                  <c:choose>
                  	<c:when test="${tPro.tState==3 }">
                  		<a href="${pageContext.request.contextPath }/sch/seeDataOfCol.do?pId=${tPro.pId }&cId=${tPro.cId }&currentPage=1" class="am-btn am-btn-default am-btn-xs am-text-success"><span class="am-icon-adn "></span>&nbsp;查看</a>
                  		<a href="${pageContext.request.contextPath }/sch/affirm.do?tId=${tPro.tId }&pId=${tPro.pId }" class="am-btn am-btn-default am-btn-xs am-text-success" onclick="return confirm('确认接收？')"><span class="am-icon-adn "></span>&nbsp;确认</a>
                  		<a href="${pageContext.request.contextPath }/sch/returnPage.do?tId=${tPro.tId }&pId=${tPro.pId }&cId=${tPro.cId }" class="am-btn am-btn-default am-btn-xs am-text-success" ><span class="am-icon-adn "></span>&nbsp;驳回</a>
                  	</c:when>
                  	<c:when test="${tPro.tState==5 }">
                  		<font color="red">理由：${tPro.backMsg }</font>
                  	</c:when>
                  	<c:when test="${tPro.tState==4 }">
                  		<font color="red">已确认</font>
                  		<a href="${pageContext.request.contextPath }/sch/returnPage.do?tId=${tPro.tId }&pId=${tPro.pId }&cId=${tPro.cId }" class="am-btn am-btn-default am-btn-xs am-text-success" ><span class="am-icon-adn "></span>&nbsp;驳回</a>
